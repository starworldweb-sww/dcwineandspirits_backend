...
const parseCategoryFilters = (query) => {
  ...
  const subcategoryIds = parseIdList(
    query.fc ?? query.subcategory ?? query.category_id,
  );
...
const buildFilterList = async (productIds, selectedIds) => {
  ...
  return groups
    .map((g) => {
      const groupDesc = groupDescriptions.find(
        (gd) => gd.filter_group_id === g.filter_group_id,
      );
      ...
      return {
        ...
        filters: filters
          ...
          .map((f) => {
            const desc = filterDescriptions.find(
              (fd) => fd.filter_id === f.filter_id,
            );
            return {
              id: f.filter_id,
              name: desc?.name || "",
...
const applyDisplayOrder = (
  products,
  sort,
  priceGetter = (p) => Number(p.final_price),
) => {
...
const buildBrandList = (allProducts, manufacturers, seoMap, selectedIds) => {
  ...
  return manufacturers.map((m) => {
    const count = allProducts.filter(
      (p) => p.manufacturer_id === m.manufacturer_id,
    ).length;
    return {
...
const buildOptionList = async (productIds, selectedIds) => {
  ...
  const optionValueIds = [
    ...new Set(productOptions.map((po) => po.option_value_id)),
  ];

  if (!optionIds.length) return [];

  const [options, optionDescriptions, optionValues, optionValueDescriptions] =
    await Promise.all([
    ...
    .map((o) => {
      ...
      return {
        ...
        filters: optionValues
          ...
          .map((ov) => {
            const desc = optionValueDescriptions.find(
              (ovd) => ovd.option_value_id === ov.option_value_id,
            );
            return {
...
const filterProducts = (items, filters) => {
  ...
  if (minPrice != null) {
    filtered = filtered.filter(
      (p) => getEffectivePrice(p.price, p.special_price) >= minPrice,
    );
  }

  if (maxPrice != null) {
    filtered = filtered.filter(
      (p) => getEffectivePrice(p.price, p.special_price) <= maxPrice,
    );
  }
...
const allProductsData = async () => {
  ...
  const isDateActive = (start, end) => {
    ...
    const startOk =
      !start || Number.isNaN(startYear) || startYear <= 1970 || start <= now;
    const endOk =
      !end || Number.isNaN(endYear) || endYear <= 1970 || end >= now;
...
// productIds ke liye review count + average rating ek saath nikalta hai
const getReviewSummaryMap = async (productIds) => {
  if (!productIds.length) return new Map();

  const reviewStats = await prisma.oc_review.groupBy({
    by: ["product_id"],
  ...
  return new Map(
    reviewStats.map((r) => [
      r.product_id,
      {
        count: r._count.review_id,
        avg: r._avg.rating ? Number(r._avg.rating.toFixed(1)) : 0,
      },
    ]),
  );
};
...
const getProductsSummary = async (productIds) => {
    ...
    const [products, descriptions, seoUrls, specials, manufacturers, reviewMap] = await Promise.all([
        ...
        getReviewSummaryMap(productIds),
    ]);
    ...
    const data = products.map((p) => {
        ...
        const reviewData = reviewMap.get(p.product_id);
        ...
        return {
            ...
            average_rating: reviewData?.avg ?? 0,
        };
    });
...
const getCategoryData = async (categoryId, query) => {
  ...
  const [category, categoryDesc, subCategories, productLinks, breadcrumbs] =
    await Promise.all([
  ...
  const subCategoryDescMap = new Map(
    subCategoryDescs.map((d) => [d.category_id, d.name]),
  );
  ...
  const cleanedProducts = sortedProducts.map(
    ({ id, category_ids, sort_order, ...rest }) => rest,
  );
...
const getManufacturerData = async (manufacturerId, query = {}) => {
  ...
  const [manufacturer, manufacturerDesc, productRows] = await Promise.all([
    prisma.oc_manufacturer.findUnique({
...
`;
  ...
  const optionValueDescs = allOptionValueIds.length
    ? await prisma.oc_option_value_description.findMany({
        where: {
          option_value_id: { in: allOptionValueIds },
          language_id: LANGUAGE_ID,
        },
      })
    : [];
  const optionValueDescMap = new Map(
    optionValueDescs.map((d) => [d.option_value_id, d.name]),
  );
  ...
  const attributeDescs = attributeIds.length
    ...
    : [];
  const attributeDescMap = new Map(
    attributeDescs.map((d) => [d.attribute_id, d.name]),
  );
  ...
  const manufacturer_seo = await prisma.oc_seo_url.findFirst({
    where: {
      query: `manufacturer_id=${manufacturer?.manufacturer_id}`,
      store_id: STORE_ID,
      language_id: LANGUAGE_ID,
    },
    select: { keyword: true },
  ...
  return {
    ...
    manufacturer: manufacturer
      ? {
          ...
          manufacturer_seo_url: manufacturer_seo?.keyword ?? null,
        }
      : null,
...
export const getSearchResultsService = async (query) => {
  ...
  const allManufacturerIds = [
    ...new Set(
      catalogProducts.map((p) => p.manufacturer_id).filter((id) => id > 0),
    ),
  ];
  ...
  const filteredPrices = filtered
    .map((item) => getEffectivePrice(item?.price, item?.special_price))
    .filter(Number.isFinite);
  ...
  const sortedFiltered = applyDisplayOrder(filtered, sort, (p) =>
    getEffectivePrice(p.price, p.special_price),
  );
  ...
  return {
    ...
    products: {
      ...
      items: paginated.items.map(
        ({ category_ids, sort_order, ...product }) => product,
      ),
    },
  };
};
...
export const mostviewdproductservice = async () => {
  ...
  const flatItems = result.map(
    ({ oc_product_description, oc_product_special, ...product }) => {
  ...
  const ordered = applyDisplayOrder(flatItems, "default", (p) =>
    Number(p.special_price ?? p.original_price),
  );
